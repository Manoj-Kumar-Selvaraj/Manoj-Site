output "cloudfront_domain" {
  description = "CloudFront distribution domain name — this is your frontend URL"
...
output "django_admin_url" {
  ...
  value       = "http://${aws_eip.portfolio.public_ip}/admin/"
}

output "api_base_url" {
  description = "Backend API base URL — set as API_BASE_URL in GitHub Secrets"
  value       = "http://${aws_eip.portfolio.public_ip}/api"
}
...
output "github_secrets_summary" {
  ...
  value = <<-EOT
    ...
    │  API_BASE_URL                        │  http://${aws_eip.portfolio.public_ip}/api
